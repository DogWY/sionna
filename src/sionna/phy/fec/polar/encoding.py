...
class Polar5GEncoder(PolarEncoder):
    ...
    def input_interleaver(self, c: np.ndarray) -> np.ndarray:
        """Input interleaver following Sec. 5.4.1.1 in :cite:p:`3GPPTS38212`.

        :param c: 1D array to be interleaved.

        :output c_apo: Interleaved version of ``c`` with same shape and
            dtype as ``c``.
        """
        ...
        k = len(c)
        if k > k_il_max:
            msg = "Input interleaver only defined for length of 164."
            raise ValueError(msg)
